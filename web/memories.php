...
// mem0ry4ai web UI — memories list: filters + FTS + grouped table + bulk + CRUD (AJAX).
...
<tr class="<?= $st === 'superseded' ? 'row-superseded' : '' ?>" data-id="<?= h($r['id']) ?>" data-status="<?= h($st) ?>"
  ...
  <td><a class="scope-tag" href="<?= $scope === 'global' ? 'memories.php?scope=global' : 'project.php?slug=' . h(scope_label($scope)) ?>"><?= h(scope_label($scope)) ?></a></td>
  <td class="summary" onclick="toggleBody(this)">
    ...
    <?php if ($st === 'superseded'): ?><span class="status-superseded">· superseded<?= !empty($m['superseded-by']) ? ' → <a href="memories.php?id=' . h($m['superseded-by']) . '">' . h($m['superseded-by']) . '</a>' : '' ?></span><?php endif; ?>
    <div class="meta"><a href="memories.php?id=<?= h($r['id']) ?>"><?= h($r['id']) ?></a> · conf <?= h($m['confidence'] ?? '?') ?> · <?= h($m['source'] ?? '') ?></div>
  </td>
  ...
  <td style="text-align:right">
    <div class="actwrap">
      ...
      <div class="actmenu">
        ...
        <a href="memories.php?id=<?= h($r['id']) ?>"><?= t('View chain') ?></a>
...
$qs = function (array $over) use ($q, $fScope, $fType, $fStat) {
    ...
    return 'memories.php?' . http_build_query(array_merge($p, $over));
};
...
<title><?= t('Memories') ?> — mem0ry4ai</title>
...
<body>
<?= render_topbar('memories') ?>

<main>
<div class="layout">
<div class="content">
  <div class="crumb"><a href="index.php"><?= t('Dashboard') ?></a> / <?= t('Memories') ?></div>
  ...
  <div class="chain"><b><?= t('Supersede chain:') ?></b>
    <?php foreach ($chainIds as $i => $cid): ?><?= $i ? '<span class="arrow">→</span>' : '' ?><a href="memories.php?id=<?= h($cid) ?>"><?= h($cid) ?></a><?php endforeach; ?>
    <a class="btn btn-ghost" style="margin-left:12px" href="memories.php"><?= t('close') ?></a>
  </div>
  <?php elseif ($fId !== ''): ?>
  <div class="chain"><code><?= h($fId) ?></code> <?= t('(no chain)') ?>. <a href="memories.php"><?= t('back') ?></a></div>
  <?php endif; ?>
...
form.addEventListener('submit', function(e){
  ...
  fetch('memories.php', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' }, body: fd })
...
function postAction(action, id, extra){
  ...
  return fetch('memories.php', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' }, body: fd })
    .then(function(r){ return r.json(); })
...
/* ---------- live poll: the store-changed banner (table not auto-patched) ---------- */
var pollVer = '';
var pollBusy = false;
function pollStore(){
  ...
  fetch('poll.php?ver=' + encodeURIComponent(pollVer))
    ...
    .then(function(j){
      ...
      var nb = document.getElementById('newbanner'); if (nb) nb.style.display = '';
    })
